...
//    }
    ...
    public List<StatisticRepairmanDTO> getTopRepairMan(String startDate, String endDate) {
        String sql = "SELECT users.full_name as tenNV, count(c.code) as sophieuHT FROM maintenance_cards c  " +
                " inner join users on c.repairman_id = users.id  " +
                " where c.work_status = 2  and c.repairman_id != 0 " +
